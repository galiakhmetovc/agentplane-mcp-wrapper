...
Локальный MCP-wrapper для подключения Codex, Claude и IDE-агентов к демо
AgentPlane.

В этом репозитории только клиентский адаптер. Policy service, gateway,
Execution Point, identity provider, аудит и веб-консоль работают на стороне
AgentPlane.
...
Если локальная сессия отсутствует, wrapper вернет ссылку вида:

```text
https://agentplane.tech/idp/realms/demo/device?user_code=....
```

Откройте ссылку, подтвердите код и повторите тот же запрос агенту. Пароль не
передается wrapper-у или агенту.

## Что делает wrapper

- запускается как stdio MCP server на рабочей машине пользователя;
- получает пользовательскую OIDC-сессию через device code или browser PKCE;
